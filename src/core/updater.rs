...
use crate::core::{
    fridgerator::{GITHUB_API, REPO_PATH},
    gui::SimpleYesNoDialog,
    http, Error, Fridgerator, Gui,
};
...
impl Updater {
    ...
    fn check_for_updates_internal(&self) -> Result<bool, Error> {
        ...
        if let Some(mutex) = Gui::instance() {
            mutex
                .lock()
                .unwrap()
                .show_notification(&t!("notification.checking_for_updates"));
        }

        let latest =
            http::get_json::<Release>(&format!("{}/{}/releases/latest", GITHUB_API, REPO_PATH))?;
        ...
        if latest.is_different_version() {
            #[cfg(target_os = "windows")]
            {
                let installer_asset = latest
                    .assets
                    .iter()
                    .find(|asset| asset.name == "fridgerator_installer.exe");
                        ...
                        mutex
                            ...
                            .show_window(Box::new(SimpleYesNoDialog::new(
                                ...
                                |ok| {
                                    ...
                                    Fridgerator::instance().updater.clone().run();
                                },
                            )));
                    }
                    return Ok(true);
    ...
    pub fn run(self: Arc<Self>) {
        ...
        #[cfg(target_os = "android")]
        {
            use crate::{
                android::utils,
                core::fridgerator::{UMAPATCHER_INSTALL_URL, UMAPATCHER_PACKAGE_NAME},
            };
    ...
    fn run_internal(self: Arc<Self>) -> Result<(), Error> {
        ...
        let fridgerator_path_str =
            unsafe { widestring::Utf16Str::from_slice_unchecked(&slice[..length]) };
        ...
        unsafe {
            ShellExecuteW(
                ...
                &HSTRING::from(format!(
                    ...
                    game_dir.display(), fridgerator_path_str, std::env::args().skip(1).collect::<Vec<String>>().join(" ")
                )),
...
#[derive(Deserialize)]
struct Blake3Hashes {
    #[serde(rename = "fridgerator_installer.exe")]
