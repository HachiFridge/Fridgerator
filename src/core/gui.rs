...
pub fn register_plugin_menu_icon(label: String, uri: String, bytes: Vec<u8>) -> bool {
    ...
    PLUGIN_MENU_ICONS.lock().unwrap().insert(label, PluginMenuIcon { uri, bytes: bytes.into() });
    true
}
...
impl Gui {
    ...
    pub fn instance_or_init(open_key_id: &str) -> &Mutex<Gui> {
        ...
        let mut fps_value = fridgerator.target_fps.load(atomic::Ordering::Relaxed);
        if fps_value == -1 {
            fps_value = 30;
        }
        ...
        let instance = Gui {
            ...
            splash_sub_str: {
                #[cfg(target_os = "windows")]
                {
                    let key_label = crate::windows::utils::vk_to_display_label(
                        fridgerator.config.load().windows.menu_open_key,
                    );
            ...
            #[cfg(target_os = "windows")]
            menu_vsync_value: fridgerator.vsync_count.load(atomic::Ordering::Relaxed),
        ...
        unsafe {
            ...
            fridgerator.run_auto_update_check();
    ...
    pub fn run(&mut self) -> egui::FullOutput {
        ...
        let live_scale = Fridgerator::instance().config.load().gui_scale;
    ...
    fn run_splash(&mut self) {
        ...
        egui::Area::new(id)
            .fixed_pos(egui::Pos2 { x: (-250.0 * scale) * (1.0 - tween_val), y: 16.0 * scale })
            .show(ctx, |ui| {
                egui::Frame::NONE
                    ...
                    .show(ui, |ui| {
                        ui.horizontal(|ui| {
                            ...
                            ui.heading("Fridgerator");
                            ui.label(env!("FRIDGERATOR_DISPLAY_VERSION"));
                        });
    ...
    fn run_menu(&mut self) {
        ...
        let mut show_window: Option<BoxedWindow> = None;
        {
            ...
            egui::SidePanel::left(egui::Id::new("fridgerator_menu").with(salt.to_bits()))
                    ...
                    ui.with_layout(egui::Layout::top_down_justified(egui::Align::TOP), |ui| {
                        ...
                        egui::ScrollArea::vertical().show(ui, |ui| {
                            ...
                            ui.horizontal(|ui| {
                                ...
                                let res =
                                    ui.add(egui::Slider::new(&mut self.menu_fps_value, 30..=240));
                            ...
                            #[cfg(target_os = "windows")]
                            {
                                ...
                                ui.horizontal(|ui| {
                                    ...
                                    if ui.checkbox(&mut value, "").changed() {
                                        ...
                                        Thread::main_thread().schedule(|| {
                                            let topmost = Fridgerator::instance()
                                                .window_always_on_top
                                                .load(atomic::Ordering::Relaxed);
                                            unsafe {
                                ...
                                ui.horizontal(|ui| {
                                    let mut value =
                                        fridgerator.discord_rpc.load(atomic::Ordering::Relaxed);
                            ...
                            if ui.button(t!("menu.check_for_updates")).clicked() {
                                fridgerator.tl_updater.clone().check_for_updates(false, false);
                            }
                            if ui.button(t!("menu.check_for_updates_pedantic")).clicked() {
                                fridgerator.tl_updater.clone().check_for_updates(true, false);
                            }
                            if fridgerator.config.load().translator_mode {
                                if ui.button(t!("menu.dump_localize_dict")).clicked() {
                                    Thread::main_thread().schedule(|| {
                                        let data = Localize::dump_strings();
                                        let dict_path = Fridgerator::instance()
    ...
    fn run_combo<T: PartialEq + Copy>(
        ...
        egui::ComboBox::new(ui.id().with(id_child), "").selected_text(selected).show_ui(ui, |ui| {
            for choice in choices.iter() {
    ...
    fn run_update_progress(&mut self) {
        ...
        let progress = Fridgerator::instance().tl_updater.progress().unwrap_or_else(|| {
        ...
        egui::Area::new("update_progress".into())
            .fixed_pos(egui::Pos2 { x: 4.0 * scale, y: 4.0 * scale })
            .show(ctx, |ui| {
                egui::Frame::NONE
                    .fill(BACKGROUND_COLOR)
...
impl ConfigEditor {
    pub fn new() -> ConfigEditor {
        ConfigEditor {
            config: (**Fridgerator::instance().config.load()).clone(),
            id: random_id(),
    ...
    fn restore_defaults(&mut self) {
        ...
        self.config = fridgerator::Config::default();
    ...
    fn run_options_grid(config: &mut fridgerator::Config, ui: &mut egui::Ui, tab: ConfigEditorTab) {
        ...
        match tab {
            ConfigEditorTab::General => {
                ...
                if ui.checkbox(&mut config.disable_gui, "").clicked() {
                    if config.disable_gui {
                        thread::spawn(|| {
                            Gui::instance().unwrap().lock().unwrap().show_window(Box::new(
                                SimpleOkDialog::new(
                                    &t!("warning"),
                ...
                Gui::run_combo(
                    ...
                    &[
                        (crate::core::fridgerator::BgUpdateMode::Disabled, &t!("disabled")),
                        (
                            crate::core::fridgerator::BgUpdateMode::Periodic,
                            &t!("config_editor.bg_update_periodic"),
                        ),
                        (
                            crate::core::fridgerator::BgUpdateMode::Silent,
                ...
                if config.bg_update_mode != crate::core::fridgerator::BgUpdateMode::Disabled {
            ...
            ConfigEditorTab::Graphics => {
                ...
                #[cfg(target_os = "windows")]
                {
                    use crate::windows::fridgerator_impl::{FullScreenMode, ResolutionScaling};
...
impl Window for ConfigEditor {
    fn run(&mut self, ctx: &egui::Context) -> bool {
        ...
        {
            config.windows.menu_open_key =
                Fridgerator::instance().config.load().windows.menu_open_key;
        }
        let mut reset_clicked = false;

        new_window(ctx, self.id, t!("config_editor.title")).open(&mut open).show(ctx, |ui| {
            simple_window_layout(
                ...
                |ui| {
                    egui::ScrollArea::horizontal().id_salt("tabs_scroll").show(ui, |ui| {
                        ui.horizontal(|ui| {
                            ...
                            for (tab, label) in ConfigEditorTab::display_list() {
                                if ui
                                    .selectable_label(self.current_tab == tab, label.as_ref())
                                    .clicked()
                                {
                    ...
                    egui::ScrollArea::vertical().id_salt("body_scroll").show(ui, |ui| {
                        egui::Frame::NONE.inner_margin(egui::Margin::symmetric(8, 0)).show(
                            ui,
                            |ui| {
        ...
        open &= open2;
        if !open {
            let config_locale = Fridgerator::instance().config.load().language.locale_str();
...
fn save_and_reload_config(config: fridgerator::Config) {
    let notif = match Fridgerator::instance().save_and_reload_config(config) {
    ...
    thread::spawn(move || {
        Gui::instance().unwrap().lock().unwrap().show_notification(&notif);
    });
}
...
impl Window for FirstTimeSetupWindow {
    fn run(&mut self, ctx: &egui::Context) -> bool {
        ...
        new_window(ctx, self.id, t!("first_time_setup.title")).open(&mut open).show(ctx, |ui| {
            let allow_next = match self.current_page {
                1 => (**self.index_request.result.load()).as_ref().map_or(false, |r| r.is_ok()),
                _ => true,
            };
                    ...
                    match i {
                        ...
                        1 => {
                            ...
                            async_request_ui_content(
                                ...
                                |ui, repo_list| {
                                    let fridgerator = Fridgerator::instance();
                                    let current_lang_str =
                                        fridgerator.config.load().language.locale_str();

                                    let mut filtered_repos: Vec<_> = repo_list
                                        .iter()
                                        .filter(|repo| repo.region == fridgerator.game.region)
                                        .collect();
                                    ...
                                    filtered_repos
                                        .sort_by_key(|repo| !repo.is_recommended(current_lang_str));
                                        ...
                                        egui::Frame::NONE
                                            ...
                                            .show(ui, |ui| {
                                                ...
                                                for repo in filtered_repos.iter() {
                                                    ...
                                                    let is_selected = self.current_tl_repo.as_ref()
                                                        == Some(&repo.index);
...
impl Window for AboutWindow {
    fn run(&mut self, ctx: &egui::Context) -> bool {
        ...
        new_window(ctx, self.id, t!("about.title")).open(&mut open).show(ctx, |ui| {
            ui.horizontal(|ui| {
            ...
            ui.horizontal(|ui| {
                ...
                if ui.button(t!("about.check_for_updates")).clicked() {
                    Fridgerator::instance().updater.clone().check_for_updates(|_| {});
                }
            });
        });
...
impl Window for LicenseWindow {
    fn run(&mut self, ctx: &egui::Context) -> bool {
        ...
        new_window(ctx, self.id, t!("license.title")).open(&mut open).show(ctx, |ui| {
            egui::ScrollArea::vertical().show(ui, |ui| {
