...
use super::{
    fridgerator::{self, Language, REPO_PATH, WEBSITE_URL},
    http::AsyncRequest,
...
static UNITY_COLORS: std::sync::OnceLock<FnvHashMap<&'static str, Color32>> =
...
#[cfg(target_os = "android")]
static KEYBOARD_SELECTION: std::sync::LazyLock<Mutex<RangeInt>> =
    std::sync::LazyLock::new(|| Mutex::new(RangeInt::new(0, 1)));
        ...
        *KEYBOARD_SELECTION.lock().unwrap() = initial_selection;
        ...
        Thread::main_thread().schedule(|| {
            ...
            let typ: TouchScreenKeyboardType::KeyboardType = unsafe {
                *(&PENDING_KB_TYPE.load(Ordering::Relaxed) as *const i32
...
impl Gui {
    ...
    pub fn run(&mut self) -> egui::FullOutput {
        ...
        if fridgerator::CONFIG_LOAD_ERROR.swap(false, Ordering::Relaxed) {
                    ...
                    ui.with_layout(egui::Layout::top_down_justified(egui::Align::TOP), |ui| {
                        ...
                        egui::ScrollArea::vertical().show(ui, |ui| {
                            ...
                            if ui
                                ...
                                .clicked()
                            {
                                fridgerator.tl_updater.clone().check_for_updates(true);
                            }
    ...
    fn run_update_progress(&mut self) {
        ...
        let progress = Fridgerator::instance()
            .tl_updater
            .progress()
            .unwrap_or_else(|| {
...
struct StyleState {
    color: Color32,
    size: f32, // had to remove sadly due to unnecessary bloat of font files
               // bold: bool,
               // italic: bool
}
    ...
    fn run(&mut self, ctx: &egui::Context) -> bool {
        ...
        let window_res = new_window(ctx, self.id, "")
            ...
            .show(ctx, |ui| {
                ...
                ui.vertical(|ui| {
                    egui::Frame::NONE.inner_margin(10.0 * scale).show(ui, |ui| {
                        egui::ScrollArea::vertical().show(ui, |ui| {
                            ui.horizontal_wrapped(|ui| {
                                rich_text_label(
                                    ui,
                                    &format!("<b><size=16>{}</bold></b>", self.name),
                                );
                                rich_text_label(
                                    ui,
                                    &format!("#<b>{}</b>", self.skill_id.to_string()),
                                );
                            });
...
impl ConfigEditor {
    pub fn new() -> ConfigEditor {
        let handle = Fridgerator::instance().config.load();
        ConfigEditor {
    ...
    fn run_options_grid(config: &mut fridgerator::Config, ui: &mut egui::Ui, tab: ConfigEditorTab) {
        ...
        match tab {
            ...
            ConfigEditorTab::Graphics => {
                ...
                let _res = ui.add(
                    egui::Slider::new(&mut config.ui_animation_scale, 0.1..=10.0).step_by(0.1),
                );
                #[cfg(target_os = "android")]
            ...
            ConfigEditorTab::Gameplay => {
                ...
                Gui::run_combo(
                    ...
                    &[
                        ...
                        (
                            BgSeason::Spring,
                            unsafe { (*Localize::Get(1067)).as_utf16str() }
                                .to_string()
                                .as_str(),
                        ),
                        (
                            BgSeason::Summer,
                            unsafe { (*Localize::Get(1068)).as_utf16str() }
                                .to_string()
                                .as_str(),
                        ),
                        (
                            BgSeason::Fall,
                            unsafe { (*Localize::Get(1069)).as_utf16str() }
                                .to_string()
                                .as_str(),
                        ),
                        (
                            BgSeason::Winter,
                            unsafe { (*Localize::Get(1070)).as_utf16str() }
                                .to_string()
                                .as_str(),
                        ),
                        (
                            BgSeason::CherryBlossom,
                            unsafe { (*Localize::Get(1071)).as_utf16str() }
                                .to_string()
                                .as_str(),
                        ),
                    ],
...
impl Window for ConfigEditor {
    fn run(&mut self, ctx: &egui::Context) -> bool {
        ...
        let mut open = true;
        let mut open2 = true;
        let global_handle = Fridgerator::instance().config.load();
...
impl FirstTimeSetupWindow {
    fn new() -> FirstTimeSetupWindow {
        let config = (**Fridgerator::instance().config.load()).clone();
...
impl LiveVocalsSwapWindow {
    fn new() -> LiveVocalsSwapWindow {
        let fridgerator = Fridgerator::instance();
        let chara_data = fridgerator.chara_data.load_full();
        ...
        LiveVocalsSwapWindow {
            id: random_id(),
            config: (**fridgerator.config.load()).clone(),
            chara_choices,
