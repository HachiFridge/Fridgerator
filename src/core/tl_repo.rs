...
use super::{
    ...
    fridgerator::LocalizedData,
    http::{self, AsyncRequest},
    utils, Error, Gui, Fridgerator,
};
...
impl DownloadJob {
    fn new(agent1: ureq::Agent) -> DownloadJob {
        DownloadJob { agent: agent1, hasher: blake3::Hasher::new(), buffer: [0u8; CHUNK_SIZE] }
    }
}
...
impl Updater {
    ...
    fn check_for_updates_internal(&self, pedantic: bool, silent: bool) -> Result<(), Error> {
        ...
        if let Ok(head_res) = ureq::agent().head(index_url).call() {
            if let Some(etag_str) = head_res.header("ETag") {
                ...
                if let Some(cached_etag) = &repo_cache.index_etag {
                    if !pedantic && cached_etag == &etag_string {
                        debug!(
                            "Server ETag matches cached ETag. No translation updates available."
                        );
                        if !silent {
                            if let Some(mutex) = Gui::instance() {
                                mutex
            ...
            let updated = if is_new_repo {
                ...
                if let Some(path) = path {
                    ...
                    if !exists {
                        ...
                        let size_mismatch =
                            metadata.map(|m| m.len() as usize != file.size).unwrap_or(true);
        ...
        if !update_files.is_empty() {
            ...
            if silent {
                ...
                let updater = Fridgerator::instance().tl_updater.clone();
    ...
    fn download_zip(
        ...
        {
            let total_size_header =
                ureq::agent().head(&update_info.zip_url).call().ok().and_then(|res| {
                    res.header("Content-Length").and_then(|s| s.parse::<usize>().ok())
                });
