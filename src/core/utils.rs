...
fn custom_wrap_algorithm<'a, 'b>(
    ...
    let config = &Fridgerator::instance().localized_data.load();
...
pub fn fit_text(string: &str, base_line_width: i32, base_font_size: i32) -> Option<String> {
    let mult = Fridgerator::instance().localized_data.load().config.line_width_multiplier?;
...
pub fn wrap_fit_text_il2cpp(
    ...
    if Fridgerator::instance().localized_data.load().config.use_text_wrapper {
...
pub fn truncate_chars(
    ...
    let line_width_multiplier =
        Fridgerator::instance().localized_data.load().config.line_width_multiplier?;
...
pub fn load_rgba_png<R: std::io::Read + std::io::Seek>(r: R) -> Option<(Vec<u8>, png::OutputInfo)> {
    let mut reader = png::Decoder::new(std::io::BufReader::new(r)).read_info().ok()?;
