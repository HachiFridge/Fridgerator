...
use crate::core::{gui::{PersistentMessageWindow, SimpleYesNoDialog}, http, Error, Gui, Fridgerator};

use super::{main::DLL_HMODULE, utils};

const REPO_PATH: &str = "THShafi170/Fridgerator";
...
impl Updater {
    ...
    fn check_for_updates_internal(&self) -> Result<bool, Error> {
        ...
        if latest.is_different_version() {
            let installer_asset = latest.assets.iter().find(|asset| asset.name == "fridgerator_installer.exe");
    ...
    fn run_internal(self: Arc<Self>) -> Result<(), Error> {
        ...
        let fridgerator_path_str = unsafe { Utf16Str::from_slice_unchecked(&slice[..length]) };
