...
extern "C" fn SetFontSize(this: *mut Il2CppObject, font_size: i32) {
    ...
    if let Some(mult) = Fridgerator::instance().localized_data.load().config.text_frame_font_size_multiplier {
...
pub fn init(umamusume: *const Il2CppImage) {
    ...
    // This is also StoryViewTextControllerShortStory::SetFontSize due to compiler optimizations
    let SetFontSize_addr = get_method_addr(StoryViewTextControllerSingleMode, c"SetFontSize", 1);

    new_hook!(SetFontSize_addr, SetFontSize);

    unsafe {
        _TEXTFRAME_FIELD = get_field_from_name(StoryViewTextControllerSingleMode, c"_textFrame");
    }
}