...
use crate::{android::gui_impl::input_hook, core::{Error, Fridgerator, Interceptor}};
use super::utils;
...
extern "C" fn dlopen(filename: *const c_char, flags: c_int) -> *mut c_void {
    ...
    let orig_fn: DlopenFn = unsafe {
        std::mem::transmute(fridgerator.interceptor.get_trampoline_addr(dlopen as usize))
    };
    ...
    if fridgerator.on_dlopen(filename_str, handle as usize) {
        fridgerator.interceptor.unhook(dlopen as usize);
    }
...
extern "C" fn do_dlopen(filename: *const c_char, flags: c_int, extinfo: *const c_void, caller_addr: *const c_void) -> *mut c_void {
    ...
    let orig_fn: DoDlopenFn = unsafe {
        std::mem::transmute(fridgerator.interceptor.get_trampoline_addr(do_dlopen as usize))
    };
    ...
    let filename_str = unsafe { CStr::from_ptr(filename).to_str().unwrap() };
    if fridgerator.on_dlopen(filename_str, handle as usize) {
        fridgerator.interceptor.unhook(do_dlopen as usize);
    }
...
extern "C" fn JNINativeInterface_RegisterNatives(env: JNIEnv, class: jclass, methods_: *const JNINativeMethod, count: jint) -> jint {
    ...
    let orig_fn: RegisterNativesFn = unsafe {
        std::mem::transmute(fridgerator.interceptor.get_trampoline_addr(JNINativeInterface_RegisterNatives as usize))
    };
    ...
    for method in methods {
        ...
        if name == "nativeInjectEvent" {
            ...
            fridgerator.interceptor.unhook(JNINativeInterface_RegisterNatives as usize);
        }
    }
...
fn init_internal(env: *mut jni::sys::JNIEnv) -> Result<(), Error> {
    ...
    let force_hook_dlopen = fridgerator.config.load().android.hook_libc_dlopen ||
    ...
    fridgerator.interceptor.hook(dlopen_orig, dlopen_hook)?;

    if !fridgerator.config.load().disable_gui {
        info!("Hooking JNINativeInterface RegisterNatives");
        let register_natives_addr = unsafe { (**env).RegisterNatives.unwrap() as usize };
        fridgerator.interceptor.hook(register_natives_addr, JNINativeInterface_RegisterNatives as usize)?;
    }
